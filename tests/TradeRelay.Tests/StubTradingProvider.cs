...
internal sealed class StubTradingProvider : IExchangeTradingProvider
{
    ...
    public Task<OperationResult> CancelAllOrdersAsync(string? symbol, CancellationToken cancellationToken) => Task.FromResult(new OperationResult(true, "OK", "Cancelled all.", 0, ExchangeOrderStatus.Cancelled, DateTimeOffset.UtcNow));
    public Task<OrderSubmissionResult> ClosePositionAsync(ClosePositionRequest request, CancellationToken cancellationToken) => Task.FromResult(new OrderSubmissionResult(true, true, "close-1", "close-client", ExchangeOrderStatus.Filled, request.Quantity, request.Quantity, 0m, 100m, "Closed.", DateTimeOffset.UtcNow));
